...
oauth2_scheme = OAuth2PasswordBearer(
    tokenUrl="aapi/uth/token", description="Enter your access token"
)

# Alternative: HTTP Bearer for header-based auth
security = HTTPBearer(scheme_name="Bearer Token", description="Enter your bearer token")
...
# Include routers
app.include_router(
    stock_controller.router, prefix="/api", dependencies=[Depends(security)]
)
app.include_router(
    market_controller.router, prefix="/api", dependencies=[Depends(security)]
)
app.include_router(
    broker_controller.router, prefix="/api", dependencies=[Depends(security)]
)
app.include_router(
    nse_controller.router, prefix="/api", dependencies=[Depends(security)]
)
app.include_router(
    ai_pdf_agents_controller.router, prefix="/api", dependencies=[Depends(security)]
)
app.include_router(auth_controller.router, prefix="/api")
